...
public class SearchActivity extends ActionBarActivity {
	...
	private class DownloadURLTask extends AsyncTask<String, Void, String> {
	 ...
	 public void onClick(View view) {
		...
		try{
		mydbnw = openOrCreateDatabase(ThirdActivity.DBNAME, Context.MODE_PRIVATE, null);
		Cursor c = mydbnw.rawQuery("SELECT * from " + ThirdActivity.TABLE, null);
		int count = c.getCount();
		c.moveToFirst();
		String bgrup;
		String locatn;
		String[] distance1=new String[count];
		String minDis = null;
		float defalultDis=20000;
		for (Integer j = 0; j < count; j++) {
			bgrup=c.getString(c.getColumnIndex("BLOOD_GROUP"));
			if(bgrup.equals(bloodgroup)){
				locatn=c.getString(c.getColumnIndex("LOCATION"));
				String s = getDirectionsUrl(locatn,location);
		DownloadURL du = new DownloadURL();
		du.execute(new String[] { s });
				Float f=Float.valueOf(dis)/1000;
				distance1[j]=f.toString();
				//minDis=distance1[j];
				if(Integer.valueOf(distance1[j])<defalultDis){
					minDis=distance1[j];
				}
				else{
					minDis=String.valueOf(defalultDis);
				}
				defalultDis=Float.valueOf(distance1[j]);
